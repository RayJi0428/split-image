...
if len(sys.argv) == 1:
    # test
    json_file = f'{os.getcwd()}\\029b99303.json'
    png_file = f'{os.getcwd()}\\129b99303.png'
else:
...
if 'frames' in json_data:
    ...
    for name in json_data['frames']:
        ...
        dst = cv2.copyMakeBorder(
            copy, oy, sh-h-oy, ox, sw-w-ox, cv2.BORDER_CONSTANT)
    ...
    for name in json_data['res']:
        ...
        # https://codertw.com/%E7%A8%8B%E5%BC%8F%E8%AA%9E%E8%A8%80/356918/
        cv2.imencode('.png', copy)[1].tofile(output_dir + name + '.png')
elif json_data[3] and json_data[3][0] == 'cc.SpriteFrame':
    idx = 0
    for frame_data in json_data[5]:
        frame = frame_data[0][0]
        name = frame['name']  # 名稱
        if name == '':
            name = str(idx)
            idx += 1
        x = frame['rect']['x']  # 位於spritesheet x座標
        y = frame['rect']['y']  # 位於spritesheet y座標
        w = frame['rect']['width']  # 位於spritesheet 寬
        h = frame['rect']['height']  # 位於spritesheet 高
        ox = frame['offset']['x']  # 位於spritesheet 裁減區和原圖x偏移量
        oy = frame['offset']['y']  # 位於spritesheet 裁減區和原圖y偏移量
        sw = frame['originalSize']['width']  # 原圖寬度(含透明區)
        sh = frame['originalSize']['height']  # 原圖高度(含透明區)
